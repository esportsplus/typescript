...
describe("exceptions channel — errorCause", () => {
    it("flags a rethrow of a new error that drops the caught cause", () => {
        const diags = analyzeFixture({
            "a.ts": `export function b(): void {
...
`,
        ...
        expect(messages(diags).some((m) => m.includes("Rethrow drops the caught error"))).toBe(true);
    });
...
describe("exceptions channel — unresolved and widening", () => {
    it("leaves an unresolved `any` callee effect-free", () => {
        const diags = analyzeFixture({
