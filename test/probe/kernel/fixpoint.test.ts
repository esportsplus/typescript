...
import { runChannel } from "~/probe/kernel/fixpoint";

import type { Channel, Diagnostic } from "~/probe/kernel/types";
...
`,
        ...
        const order: string[] = [];
        runChannel(built.analysis, orderRecorder(order), "optimist", {}, new Map());
...
describe("fixpoint — widening backstop", () => {
    it("folds an ever-growing summary to top and terminates", () => {
        built = buildAnalysis({
            "a.ts": `export function grows(): void {}
`,
        });
        const { store } = runChannel(built.analysis, everGrowing(), "optimist", {}, new Map());
