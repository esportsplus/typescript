...
import { bodyOf, constituents, isAwaited, paramSymbols } from '../kernel/ast';
...
type Env  = {
    ...
    > | undefined;
    readonly callCache: Map<
        ts.CallExpression | ts.NewExpression,
        ExceptionsValue
    > | undefined;
    readonly importsFor: (sf: ts.SourceFile) => Map<string, ImportInfo>;
    readonly namedTypeAt: (name: string, location: ts.Node) => ts.Type | undefined;
    ...
    readonly reportMode: string | undefined;
...
function createExceptionsChannel(throwIndex: ThrowIndex): Channel<ExceptionsValue> {
    ...
    const importIndex = new Map<ts.SourceFile, Map<string, ImportInfo>>();
    ...
    const importsFor = (sf: ts.SourceFile): Map<string, ImportInfo> => {
    ...
    return {
        ...
        widen,
        transfer(
            ctx: TransferContext<ExceptionsValue>,
        ): Summary<ExceptionsValue> {
            const env = makeEnv(
                ctx.checker,
                throwIndex,
                ctx.fn,
        ...
        diagnose(
            ...
            if (decl.declared && body) {
                ...
                const excess = subtract(
                    inferred,
                    new Set(decl.value.types.keys()),
                );
                if (!isEmpty(excess)) {
...
function makeEnv(
    ...
    callCache: Map<
        ...
        ExceptionsValue
    > | undefined,
    importsFor: (sf: ts.SourceFile) => Map<string, ImportInfo>,
    namedTypeAt: (
        checker: ts.TypeChecker,
        name: string,
        location: ts.Node,
...
function tryEscape(
    ...
    if (node.catchClause) {
        ...
        const discharge = dischargedKeys(
            env,
            node.catchClause,
            tryEsc,
            bindSym,
...
function resolveKeyType(env: Env, key: string): ts.Type | undefined {
    ...
    const sym = env.checker.resolveName(
        simple,
...
function callEscape(
    ...
    // No app targets and no overlay model: derive a third-party leaf from its
    // installed JavaScript.
    else if (res.targets.length === 0) {
        const derived = deriveForCall(env, call);
        if (derived) v = join(v, derived);
    }
    // An unresolved callee (`any`, a function-valued parameter/variable, an
    // abstract method) is left effect-free: modeling it as an unknown throw would
    // flag every callback/higher-order/dynamic call. Callback parameters are
    // instead handled precisely by the callback-conditional summary.
...
function consumersOnly(env: Env): boolean {
            ...
            const scan = (m: ts.Node): void => {
                ...
                if (
                    ...
                    out.push({
                        channel: 'exceptions',
                        message:
                            'Rethrow drops the caught error — pass `{ cause }` to preserve it',
...
// A call worth reporting across a package or file boundary. Calls with no app
// target are external, overlay-modeled, or unresolved and therefore cross it.
...
function reachesTop(env: Env, rem: ExceptionsValue): boolean {
    ...
    for (const o of originsOf(rem))
        if (u.origins.has(`${o.fileName}:${o.pos}`)) return true;
    return false;
}
...
function relatedFromOrigins(
    origins: ReadonlyArray<ExceptionOrigin>,
): DiagnosticRelated[] {
