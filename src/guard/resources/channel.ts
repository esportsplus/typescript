...
import { bodyOf, paramSymbols, unwrap } from '../kernel/ast';
...
type Env  = {
    ...
    readonly paramSymbols: Map<ts.Symbol, number> | undefined;
};
...
function isDischargeCall(
    expr: ts.Expression,
    acq: Acquire | undefined,
    refs: (expr: ts.Expression) => boolean,
): boolean {
...
function isReleaseCallForPair(
    ...
    acquireExpressionText: string | undefined,
): boolean {
    ...
    if (
        acquireExpressionText === undefined ||
...
function classDischargesField(
    ...
    const dischargesHere = (n: ts.Node): boolean =>
        ts.isCallExpression(n) && isDischargeCall(n, acq, refsThisField);
...
function diagnoseAcquire(
    ...
    if (binding && !binding.using) {
        ...
        if (outcome === 'leak') {
            ...
            // An escape whose transfer cannot be verified is always reported — a
            // resource that flows somewhere untrackable is treated as a leak.
            out.push({
                channel: 'resources',
...
const createResourcesChannel = (
    // Silent by default: an untrackable escape is reported directly. Callers that
    // want to measure the v1 tracking noise floor inject a logger.
