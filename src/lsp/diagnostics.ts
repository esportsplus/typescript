...
import type { Diagnostic as TscheckDiagnostic, SourceLocation } from '~/tscheck/kernel/types';

// Resolves a tscheck absolute file path to the client's open document, so ranges
...
// A tscheck SourceLocation carries 1-based line/column for display plus absolute
...
function relatedOf(diagnostic: TscheckDiagnostic, resolve: DocumentResolver, uriOf: (fileName: string) => string): DiagnosticRelatedInformation[] {
...
// Map one tscheck finding to an LSP diagnostic. The escape chain (throw site →
// boundary) rides along as relatedInformation so the editor can jump the origin.
function toLspDiagnostic(diagnostic: TscheckDiagnostic, severity: DiagnosticSeverity, resolve: DocumentResolver, uriOf: (fileName: string) => string): LspDiagnostic {
    return {
...
function groupByFile(diagnostics: ReadonlyArray<TscheckDiagnostic>, severity: DiagnosticSeverity, resolve: DocumentResolver, uriOf: (fileName: string) => string): Map<string, LspDiagnostic[]> {
